...
根据DragController的onTouchEent方法，触摸事件为MotionEvent.ACTION_UP时，拖动事件结束，这时有两种可能：当满足快速向上扔时，删除拖动的item，isFlingingToDelete方法判断条件是否满足。如果不满足扔的条件，调用drop方法，drop方法调用DropTarget的onDrop方法。
Workspace-onDrop()流程
