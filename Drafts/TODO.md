...
performReorder方法：
首先findNearestArea方法确定放下区域，这里忽略已经占据的区域，因为拖动时可能会挤开已经放下区域的item，根据mode的值确定拖动时模拟推理的向量，这个向量主要调用getDirectionVectorForDrop方法确定。接着调用findReorderSolution方法确定拖动时可能挤开别的item的解决方案，调用findConfigurationNoShuffle方法确定拖动时不影响其他item的解决方案。在代码里倾向使用拖动时对象缩小较少或不会缩小的方案，即area()的值较大者。
findReorderSolution方法,调用findNearestArea方法确定放下区域，这里应该是确定放开拖动对象时的落点，performReorder方法调用findNearestArea方法好像是多余的，我猜想应该是为了初始话result对象，避免空指针，加之使方法易读吧，必须调用的原因想不透。接着调用rearrangementExists方法确定重排方案，如果返回false，尝试分别缩小拖动对象的spanX和spanY再次findReorderSolution方法尝试,递归到minSpan，返回结果，缩小的情况是针对桌面的appWidget。
...
ViewCluster,这个类有四个数组记录了上下左右边上当前影响的Views的边界位置，用来精确的表示一串view的边界，以便于在拖动时操作View的情况。首先根据推力向量的方向计算当前受影响的边界和被推开的view的移动距离pushDistance，ViewCluster的sortConfigurationForEdgePush方法根据当前边界将拖动时受影响的View根据先后受影响的权重排序。推动时，碰到all app按钮，即canReorder为false，推动失败。推动时遍历以排好序的列表，调用isViewTouchingEdge方法判断当前View是否接触到当前受影响View集，true则加进来，一次推动一个单位距离，一直推到目标位置，拖动时影响的view就这样一层层扩大“感染”。拖动结束后，判断结果是否超过边界，不合格则restore返回推开前状态。
